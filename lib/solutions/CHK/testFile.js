    ...
    for(let groupOffer of groupOffers){
       ...
       const { items: groupOfferItems, price: groupOfferPrice, minQ: groupOfferMinQ } = groupOffer;
        ...
        if(groupCount >= groupOfferMinQ){
            ...
            let setsOfThreeTotal = setsOfThree*3;
            // Update the ItemCount:
            console.log(relevantItemCount)
            for (const [relevantItem, relevantCount] of relevantItemCount){

                console.log(relevantItem, relevantCount)
                console.log("Sets of 3:", setsOfThreeTotal)
                if (setsOfThree <= 0){
                    break;
                }
                
                setsOfThreeTotal -= relevantCount;
                console.log("ffs: ", relevantCount-setsOfThreeTotal)
                itemCount[relevantItem] = Math.max(0, relevantCount-setsOfThreeTotal);
