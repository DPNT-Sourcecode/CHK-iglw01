    ...
    for(let groupOffer of groupOffers){
       ...
       const { items: groupOfferItems, price: groupOfferPrice, minQ: groupOfferMinQ } = groupOffer;
        ...
        if(groupCount >= groupOfferMinQ){
            let setsOfOffer = Math.floor(groupCount / groupOfferMinQ);
            total += groupOfferPrice * setsOfOffer;


            let remainingSets = setsOfOffer * groupOfferMinQ
            ...
            for (const [relevantItem, relevantCount] of relevantItemCount) {
                ...
                // Calculate how many items we can use from this type (up to remainingSets)
                const itemsToUse = Math.min(relevantCount, remainingSets);

                // Subtract the used items from the itemCount
                itemCount[relevantItem] -= itemsToUse;

                // Decrease the remaining sets accordingly
                remainingSets -= itemsToUse;
            }
