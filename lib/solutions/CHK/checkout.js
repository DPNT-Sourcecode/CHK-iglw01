...
module.exports = function (skus) {
    ...
    function ComputeFreebies(letter){
        if (freebies.get(letter)){
            ...
            if(freebieLetter === letter){
                ...
                itemCount[letter] = Math.max(0, itemCount[letter]-free)
                // const rest = count -freeF;
                // total += rest*itemPrices.get(item);
            }
        }
    }
    ...
    for(let item in itemCount){
        ...
        // F:
            ...
            const freeF = Math.floor(count / 3);
            const rest = count -freeF;
            total += rest*itemPrices.get(item);
