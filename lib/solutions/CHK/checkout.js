...
module.exports = function (skus) {
    ...
    for(let groupOffer in groupOffers){
        // {items: ["S","T","X","Y","Z"], price: 45, minQ: 3},
        const [groupOfferItems, groupOfferPrice, groupOfferMinQ]= [groupOffer.items, groupOffer.price, groupOffer.minQ];

        let relevantItemCount = [];
        let groupCount = 0;
        for(const letter in groupOfferItems){
            if (itemCount[letter] > 0){
                relevantItemCount[letter] = itemCount[letter];
        ...
        if(groupCount >= groupOfferMinQ){
            ...
            for (const [item, count] of relevantItemCount){
                if (setsOfThree <= 0){
                    break;
                }
            }
